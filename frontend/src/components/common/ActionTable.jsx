...
function ActionTable({ columns, data, handleUpdate, title, handleInactive }) {
      ...
      <table className="w-full bg-black/30">
        ...
        <tbody>
          ...
          {paginatedData.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className="border-b-2 border-primary/30 hover:bg-black/30"
              onMouseOver={handleDisplayRow}
            >
              {columns.map((col, colIndex) => (
                <td key={colIndex} className="text-center text-gray-100">
                  {col === "Status" ? (
                    <span
